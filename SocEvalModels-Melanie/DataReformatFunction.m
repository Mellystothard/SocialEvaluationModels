...
%This condition is used to identify files that meet the criteria of being evaluation files.
        ...
        dataTable = readtable(filePath);  % Read the Excel file as a table. Each participant data is now referred to as dataTable
...
for raterNameCount = 1:numel(raterNames) %this goes through the list of rater names, and iterates
    ...
    if isempty(uniqueRaters) || ~ismember(rater, uniqueRaters) %isempty would return a value 1 if it is empty, and 0 if not empty.
        ...
        uniqueRaters{end+1} = rater; %'end' in Matlab indexes the last element in the array 'uniqueRaters'. So we would be adding any new
...
for blockno = 1:8 % Iterate over the blocks. I wasn't sure how to use the previous method of codes that differentiated
    %between blocks to identify the stimuli for the Hub, so I made a new
    %block counter. Do let me know if if you'd prefer to change this.
    startRow = (blockno - 1) * 20 + 2; %block starts at 1 according to counter but -1 to get a 0 based index.
                                       %multiplication by 20 gives the number of rows to skip to reach the start of the current block (since each block has 20 rows).
    ...
    blockData = dataTable(startRow:endRow, :); %the aim of this code is to go through 20 rows in the excel sheet 7 times(for 7 blocks)
...
%ratee names according to if it was a SELF or OTHER condition.
 ...
 % third element in allCodes.
    selD{ptN}.Inp.allCodes{3} = ACrateeNames;
...
selD{ptN}.Inp.allCodes{4} = ACblockTypeList; %to put the list of block types in chronological order into allcodes{4}
  ...
  % Get the block type for the block. Similar procedure and logic
    ...
    conCode = strcat(firstDigit, secondDigit, thirdDigit);
     ...
     %but here since allCodes is a repeat of the data form other fields, I the source of the data from the same set of codes.
        ...
        block = 0; % This line initializes the variable block to zero.It is used to count the blocks for each participant.
        for row = 2:size(dataTable,1)  %loop to check every row for the excel data of each participant. 160 trials altogether.
 
            if rem(row-2,rowsPerBlock) == 0 % ie we have entered a new block
                %This line checks if the current row index, adjusted by subtracting 2 (to align with the zero-based index) and divided by rowsPerBlock, has a remainder of zero.
                %It checks if the current row marks the beginning of a new block based on the specified number of rows per block.
                block = block + 1;
            end
            

            ratee_type = dataTable{row,'ratee_type'}{1} ;  %Retrieves the value of the 'ratee_type' column from the current row of the dataTable and assigns it to the variable ratee_type.
            your_name = dataTable{row,'avatar_file'}{1} ;%NEEDS CHECKING!!!  Retrieves the value of the 'avatar_file' column from the current row of the dataTable and assigns it to the variable your_name.
            selD{ptN}.Inp{block}.ratee_type = ratee_type; %Assigns the value of ratee_type to the 'ratee_type' field within the selD cell array at the appropriate participant and block indices.
                                                          %It organizes the data by participant and block, storing the ratee type.

            if strcmp(ratee_type, 'you') %If loop, if the 10th column is 'self' then extract the ratee name from the 3rd column(participant no).
                                              %if the 10th columnn is'other'then extract the ratee name from 11th column(ratee name for other conditions).
                                              %the 10th column here refers to the 'ratee_type' column in the excel file.
                                              % Which is the type of block(e.g. other or self).
               
               % table entry from col ratee_type to integer.
               %     allrateenames(row) = dataTable(row, 3); %the name of ratee for 'self' conditions would be stored in 'allrateenames'
               %                                             %which is the participant number
               selD{ptN}.Inp{block}.ratee_name = your_name; 

            elseif  strcmp(ratee_type, 'other') 
               try
                 ratee_name = dataTable{row,'other_ratee'}{1}; 
               catch %This line indicates the start of the catch block, which will be executed if an error occurs within the try block.
                 ratee_name = 'unknown'; %This line assigns the string 'unknown' to the variable ratee_name when an error occurs within the try block.
                 warning('ratee name unknown - please investigate'); %This line generates a warning message to draw attention to the fact that the ratee name is unknown and requires investigation.
                                                                     %It is triggered when an error occurs within the try block.
               end
               selD{ptN}.Inp{block}.ratee_name = ratee_name; %Assigns the value of ratee_name to the 'ratee_name' field within the selD cell array at the appropriate participant and block indices.

            selD{ptN}.Inp{block}.block = block; %To assign the value of varible block to the 'block' field in selD
