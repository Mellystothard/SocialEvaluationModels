function mL = sel_rl_L_i(P,modStruc,Inp,Resp, repoFn,details)
% FORMAT mL = sel_rl_L_i(P,modStruc,Inp,Resp)
% minus-log-likelihood function, bayesian 'rl' decision-making after beta-belief models
%             for one block of Carlisi SEL, after of Hopkins et al 21, developped from 
%             spm_mdp_L_xiii - roman num index _i, _lxix etc.
...
% Following as closely as poss the active inference format used in spm_mdp_L_xiii:
...
% repoFn % 1: just full pmf sharpening (softmax-like) ... 
%          2: also has 'inversion lapses' - pt confused which end of the VAS was which. 
%          3: flat lapses instead of 2.   is the AcIn report MDPs, 
...
for i = 1:Plen
    % first, log-transformed: ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     % logit-transformed: ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      elseif strcmp(field{i},'lps')  %  v. simple (not even a 
          % parm in MDPs) change directly:
        Pall.lps = 1/(1+exp(-P.lps));  
...
for tr = 1:trialN
  ...
  if repoFn >= 1 && repoFn <= 3  % 1: just softmax. 
                                 % 2: also has 'inversion lapses' - pt confused which end of the VAS was which. 
                                 % 3: flat lapses instead of 2.                           
      polP  = Dpos{1} .^ Pall.alphaPrec;   % as if lnP is value ;)
      polP  = polP / sum(polP);
      
      if repoFn == 2  % mix in 'inversion lapses' - pt confused which end of the VAS was which.
            polP = (1 - Pall.lps)*polP + Pall.lps*flipud(polP);
      end
      if repoFn == 3  % 'flat' lapses - all answers equally wrong
            polP = (1 - Pall.lps)*polP + Pall.lps / length(polP) ;
      end
  ...
  elseif repoFn == 0
      ...
      error(['repoFn=' nu2str(repoFn) ' not provided for yet']);
