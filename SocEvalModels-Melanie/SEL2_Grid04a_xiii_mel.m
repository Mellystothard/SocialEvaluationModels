...
% grN=1; grSc=2;     
% batch2fit=1; 
% startFitFile='genPar.mat'; 
% load(initTrParFile );
% batchSize=2;
%testGridFit = SEL2_Grid03a_xiii( batch2fit, startFitFile, grN, grSc, batchSize, fitDir, codeTest ) 
...
if isempty(fitDir)
    if ~codeTest
        fitDir = 'C:\Users\User\Desktop\UCL\Dissertation code files\SocEvalModels_(not_git)_rough_work-20230307T220354Z-001\SocEvalModels_(not_git)_rough_work';
    else
        fitDir = 'C:\Users\User\Desktop\UCL\Dissertation code files\SocEvalModels_(not_git)_rough_work-20230307T220354Z-001\SocEvalModels_(not_git)_rough_work\dataFits';
    end
...
load(data100_syntheTest);        % has selD. Data must be (re-formated) to fit w model Inp, Resp etc.
